...
定义 Flutter 客户端与官网 **light** 设计 token 的一致性要求（颜色、圆角、阴影、通用卡片与间距），以及文档化与产品验收方式；不对齐营销站深色主题，不修改 `web/` 源码。
...
The design token documentation SHALL include modal-specific tokens for solid card surfaces used in dialogs and bottom sheets, including: dialog max width rule (`min(400, viewport width − 48)` logical pixels), dialog corner radius fixed to `AppRadius.x2l` for centered dialogs, bottom sheet top corner radius using `AppRadius.x2l`, solid surface color (`AppColors.bgCard`), optional divider border (`AppColors.divider`), card shadow (`AppShadows.card`), destructive primary button colors for destructive confirms, and drag handle colors (`AppColors.divider`). The Flutter implementation SHALL match these documented values.

#### Scenario: Reviewer verifies modal tokens

- **WHEN** a reviewer compares `flutter/doc/design-tokens.md` and this specification with the modal wrapper widgets and theme
