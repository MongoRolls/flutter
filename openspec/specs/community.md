...
**描述**：每个用户拥有唯一的 6 字符好友码（字母表：`ABCDEFGHJKLMNPQRSTUVWXYZ23456789`，排除 I/O/0/1 等易混淆字符），可懒惰生成并支持轮换。
...
**场景 1：发送"喝水了吗"提醒**
- Given 用户在关怀页选择联系人并发送关怀
- When `HeartProvider.sendCare(contact)` 被调用
- Then 创建本地 `CareRecord`，触发 `NotificationService.showCareNotification()`，并在 UI 中添加到关怀时间线
...
- **BackendApiService**：`getCareContacts()`, `createCareContact()`, `getFriendCode()`, `rotateFriendCode()`, `lookupFriendCode()`
