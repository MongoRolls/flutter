...
| 个人设置 | 调整目标、提醒时段与风格；查看 AI 记住的健康信息；AI 推理默认经后端代理（密钥在服务端或构建注入，**设置页不再提供**自填 Key） |
...
1. ~~`/home` 未注册~~：**已注册**（`main.dart` 中 `'/home'` → `MainShell`），与引导完成跳转一致。
2. **对话内「设置」**：`chat_bubble` 中仍存在 `Navigator.pushNamed(context, '/settings')`，但 **`/settings` 未在 `MaterialApp.routes` 注册**，该入口当前无法打开设置页；需改为 Tab 切换、`MainShell` 切换索引或其它导航方式。
...
| **功能清单** | 问候与日期；环形进度与剩余量/目标量（单位 ml）；**喝水打卡**打开底部 sheet，选择预设杯量并记录；今日打卡次数、连续天数、已喝总量；**天气 & AI 建议**（无定位/天气时展示占位）；**今日饮水时间线**（列表）；**月度打卡日历**；底部应用版本号；**Logo 连点 5 次**进入调试页（彩蛋） |
...
| **数据与持久化** | `ChatProvider` + `AiService`（后端代理 / 运行时配置）；`UserProvider` 上下文；Hive 中与记忆/摘要相关数据由会话逻辑写入（与「健康档案」展示同源） |
| **出向跳转** | 气泡内若点击「设置」使用 `pushNamed('/settings')` — **`/settings` 未注册**，与 2.3 节已知问题一致 |
...
| **功能清单** | **基本设置**：目标 ml、体重（联动推荐目标）、提醒风格 chips、保存设置；**提醒开关**：推送总开关；**提醒时间**：起床/就寝时间选择、间隔（30/60/90/120 分钟）；**AI 健康档案**入口按钮；**模拟测试**：立即触发一条测试通知；**版本信息**（连点 5 次进调试）。**说明**：用户侧不再在设置页配置模型 API Key；AI 使用后端代理或构建时注入的密钥策略（见 `AiService` / 后端文档）。 |
| **关键交互与状态** | 保存后 SnackBar；开通知时请求系统通知权限并重新调度；关闭则取消全部调度 |
| **数据与持久化** | `UserProvider` 档案持久化并 `updateProfile` 同步后端；`NotificationService` |
...
| **出向跳转** | `pushReplacementNamed('/home')` → `MainShell`（`/home` 已在 `main.dart` 注册） |
